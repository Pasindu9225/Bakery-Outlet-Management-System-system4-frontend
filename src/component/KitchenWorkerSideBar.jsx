...
export default function KitchenWorkerSideBar({ sidebarOpen }) {
  ...
  const kitchenNavigationItems = [
    {
      ...
      path: "/kitchenWorkerDashboard",
      color: "text-blue-600",
    },
    {
      name: "Production Requests",
      icon: ClipboardList,
      path: "/kitchenProductionRequests",
      color: "text-orange-600",
    },
    {
      name: "Get Ingredients",
      icon: ShoppingBasket,
      path: "/kitchenGetIngredients",
      color: "text-green-600",
    },
    {
      name: "My Store Inventory",
      icon: Store,
      path: "/kitchenStoreInventory",
      color: "text-emerald-600",
    },
    {
      name: "Transfer Note",
      icon: ArrowLeftRight,
      path: "/kitchenTransferNote",
      color: "text-purple-600",
    },
    {
      name: "Return to Store",
      icon: RotateCcw,
      path: "/kitchenReturnToStore",
      color: "text-red-600",
    },
  ];
  ...
  return (
    ...
    >
      <div className="w-64 bg-white shadow-lg h-screen flex flex-col">
        ...
        {/* Kitchen Module Title */}
        <div className="px-4 py-3 bg-gradient-to-r from-[#0F50AA] to-[#1E40AF] text-white">
          <h2 className="text-[16px] font-[600]">Kitchen Worker Module</h2>
        ...
        <nav className="flex-1 p-4 overflow-y-auto">
          <ul className="space-y-1">
            {kitchenNavigationItems.map((item) => {
            ...
            <div className="flex-1">
              <p className="text-[14px] font-[500] text-[#383E49]">
                {localStorage.getItem("firstName") || localStorage.getItem("lastName")
                  ...
                  : localStorage.getItem("userName") || "Kitchen Worker"}
              </p>
              <p className="text-[12px] text-[#667085]">
                {getRoleName(localStorage.getItem("userRole")) || "Kitchen Staff"}
              </p>
            </div>
            <button

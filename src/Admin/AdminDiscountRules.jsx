...
export default function AdminDiscountRules() {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!trimmedName) {
            alert("Discount rule name is required");
            return;
        }
        ...
        if (nameExists) {
            alert("A discount rule with this name already exists");
            return;
        }
                ...
                <main className="flex-1 p-6 overflow-y-auto">
                    <div className="flex justify-between items-center mb-6">
                        <div>
                            ...
                            <p className="text-[14px] text-[#667085]">Manage product discounts and Happy Hour schedules</p>
                        </div>
                        <button
                            onClick={handleCreate}
                            className="bg-[#0F50AA] text-white px-4 py-2 rounded-lg flex items-center gap-2 text-[14px] font-[500]"
                        >
                    ...
                    <div className="bg-white rounded-lg shadow-sm border border-[#E4E6EA] p-4 mb-6">
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="w-full pl-10 pr-4 py-2 border border-[#E4E6EA] rounded-lg text-[14px] outline-none focus:ring-2 focus:ring-blue-500/20"
                                value={searchTerm}
                            ...
                            <div className="overflow-x-auto">
                                <table className="w-full text-left">
                                    <thead className="bg-[#F8F9FA] border-b">
                                        <tr>
                                            <th className="px-6 py-4 text-[13px] font-[600] text-[#667085]">Rule Name</th>
                                            <th className="px-6 py-4 text-[13px] font-[600] text-[#667085]">Type</th>
                                            <th className="px-6 py-4 text-[13px] font-[600] text-[#667085]">Discount</th>
                                    ...
                                    <tbody className="divide-y divide-[#E4E6EA]">
                                        {filtered.map(rule => (
                                            <tr key={rule.discountId} className="hover:bg-[#F8F9FA] group">
                                                <td className="px-6 py-4 text-[14px] font-[600] text-[#383E49]">{rule.name}</td>
                                                ...
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center justify-end gap-2">
                                                        <button onClick={() => { setSelected(rule); setModal('view'); }} className="p-2 text-gray-400 hover:text-blue-600"><Eye size={18} /></button>
                                                        <button onClick={() => handleEdit(rule)} className="p-2 text-gray-400 hover:text-green-600"><Edit size={18} /></button>
                                                        <button onClick={() => handleDelete(rule.discountId)} className="p-2 text-gray-400 hover:text-red-600"><Trash2 size={18} /></button>
                                                    </div>
                                                </td>
                                            </tr>
                                        ))}
                    ...
                    <div className="bg-white rounded-xl w-full max-w-2xl shadow-xl overflow-hidden max-h-[90vh] flex flex-col">
                        ...
                        <form onSubmit={handleSubmit} className="overflow-y-auto flex-1 p-6">
                            <div className="space-y-4">
                                <div>
                                    <label className="block text-[13px] font-[600] text-[#48505E] mb-1">Rule Name *</label>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-[13px] font-[600] text-[#48505E] mb-1">Rule Type</label>
                                    ...
                                    <div>
                                        <label className="block text-[13px] font-[600] text-[#48505E] mb-1">Discount Value *</label>
                                ...
                                {formData.ruleType === 'TIME_BASED' && (
                                    <div className="bg-blue-50 p-4 rounded-xl space-y-3">
                                        <p className="text-[12px] font-[600] text-blue-800 uppercase flex items-center gap-1"><Clock size={14}/> Schedule Setting</p>
                                        <div className="grid grid-cols-2 gap-4">
                                            <div>
                                                <label className="block text-[11px] text-blue-600 mb-1">Start Time</label>
                                                <input type="time" value={formData.startTime} onChange={e => setFormData({...formData, startTime: e.target.value})} className="w-full px-3 py-1.5 border border-blue-200 rounded" />
                                            </div>
                                            <div>
                                                <label className="block text-[11px] text-blue-600 mb-1">End Time</label>
                                                <input type="time" value={formData.endTime} onChange={e => setFormData({...formData, endTime: e.target.value})} className="w-full px-3 py-1.5 border border-blue-200 rounded" />
                                            </div>
                                        </div>
                                        <div>
                                            <label className="block text-[11px] text-blue-600 mb-1">Active Days</label>
                                ...
                                <div className="border-t pt-4">
                                    <div className="flex items-center justify-between mb-2">
                                        <label className="text-[13px] font-[600] text-[#48505E]">Target Products</label>
                                    ...
                                    {!formData.appliedToAllProducts && (
                                        <div className="border rounded-lg p-2 max-h-48 overflow-y-auto grid grid-cols-2 gap-2">
                                            {products.map(p => (
                                                <div key={p.id} onClick={() => toggleProduct(p.id)} className={`p-2 rounded border cursor-pointer text-[12px] transition-all ${formData.applicableProductIds.includes(p.id) ? 'bg-blue-50 border-blue-500 text-blue-700' : 'hover:bg-gray-50'}`}>
                                                    {p.productName}
                                                </div>
                                            ))}
                                        </div>
                            ...
                            <div className="flex gap-3 mt-6">
                                ...
                                <button type="submit" disabled={submitting} className="flex-1 px-4 py-2.5 bg-[#0F50AA] text-white rounded-lg font-[600] flex items-center justify-center gap-2">
                        ...
                        <div className="p-6 space-y-4">
                             ...
                             {selected.ruleType === 'TIME_BASED' && (
                                <div className="bg-blue-50 p-3 rounded-lg">
                                    ...
                                    <p className="text-[11px] text-blue-600 font-[500]">{Array.isArray(selected.daysOfWeek) ? selected.daysOfWeek.join(', ') : selected.daysOfWeek}</p>
                                </div>
                             )}
                             <div>

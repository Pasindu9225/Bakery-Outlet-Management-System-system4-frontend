...
function TaskDetailModal({ request, onClose, onUpdateStatus, onDispatch }) {
              ...
              <table className="w-full">
                ...
                <tbody>
                  {request.products.map((product, i) => {
                    const remaining = product.quantity - product.produced;
...
export default function BakeryProductionRequests() {
  const navigate = useNavigate();
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      try {
        ...
        console.log(`[BakeryProductionRequests] Fetching requests from ${baseUrl}/api/v1/worker/production-requests. Token: ${token ? 'present' : 'MISSING'}`);
        ...
        // Map backend shape to the existing UI shape used by the rest of the file.
        const mapped = (data || [])
  ...
  const handleUpdateStatus = async (id, newStatus) => {
    try {
      const baseUrl = process.env.REACT_APP_BASE_URL;
      const token = localStorage.getItem("authToken");
      console.log(`[BakeryProductionRequests] Updating status of ${id} to ${newStatus}. URL: ${baseUrl}/api/v1/worker/production-requests/${id}/status. Token: ${token ? 'present' : 'MISSING'}`);
  ...
  const handleDispatch = async (id) => {
    try {
      const baseUrl = process.env.REACT_APP_BASE_URL;
      const token = localStorage.getItem("authToken");
      const res = await fetch(`${baseUrl}/api/v1/worker/production-requests/${id}/dispatch`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
        },
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData.message || `Dispatch failed: ${res.status}`);
      }
      toast.success("Sent to store successfully!");
      setShowModal(false);
      // Wait for toast to be visible before reloading, or better yet, just refetch
  ...
  const STATUS_OPTIONS = [
    ...
    { value: "COMPLETED", label: "Produced" },
  ];
                  ...
                  try {
                    ...
                    const mapped = (data || [])
                      ...
                      products: (p.items || []).map((it) => ({
                        ...
                        name: it.productName,
                        quantity: it.quantity,
                        unit: p.unitOfMeasure || p.unit || "pcs",
                        produced: it.producedQuantity || 0,
                        productionCenterId: it.productionCenterId,
                      })),
                      notes: p.notes || "",
        ...
        >
          ...
          <div className="bg-surface rounded-lg shadow-sm border border-line p-6">
            <div className="flex items-center justify-between mb-5">
             ...
             {loading ? (
              <Loader variant="section" text="Loading production requests..." />
            ) : error ? (
                        ...
                        >
                          ...
                          >
                            <div className="flex items-center justify-center gap-2">
                              ...
                              {/* Distribute (100% produced or Completed, but NOT already Distributed) */}

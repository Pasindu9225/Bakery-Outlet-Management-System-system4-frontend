...
const TYPE_STYLE = {
  GRN:        { text: "text-brand-fg", bg: "bg-hover" },
  PO:         { text: "text-plum", bg: "bg-subtle" },
  Payment:    { text: "text-success", bg: "bg-hover" },
  Return:     { text: "text-warning", bg: "bg-hover" },
  Adjustment: { text: "text-fg-secondary", bg: "bg-hover" },
};
    ...
    <div className="relative">
      ...
      >
        {selected ? (
          ...
          <BookOpen size={14} className="text-fg-secondary flex-shrink-0" />
        )}
...
function FilterDropdown({ open, setOpen, value, options, onChange }) {
  return (
    <div className="relative">
      <button
        onClick={() => setOpen(!open)}
        className="flex items-center gap-2 px-3 py-2.5 border border-line rounded-lg text-[13px] text-fg bg-surface hover:bg-subtle transition-colors min-w-[150px]"
      >
    ...
    <div className="fixed inset-0 bg-backdrop bg-opacity-50 z-[9999999] flex items-center justify-center p-4">
      <div className="bg-elevated rounded-xl shadow-2xl w-full max-w-lg">
        <div className="p-5 border-b border-line flex items-center justify-between">
          <div>
            <h3 className="text-[16px] font-[600] text-fg">Document Reference</h3>
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-8 overflow-y-auto">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6">
            <div>
              <h1 className="text-[20px] font-[600] text-fg mb-1">Supplier Ledger</h1>
              <p className="text-[14px] text-fg-secondary">View, track, and reconcile supplier transactions</p>
            </div>
            ...
            {[
              {
                ...
                color: "bg-brand", hoverColor: "hover:bg-brand-hover", iconBg: "bg-brand/30",
              },
              {
                label: "Total Credit",
                value: `Rs. ${totalCredit.toLocaleString()}`,
                icon: <TrendingDown size={20} />,
                color: "bg-plum-solid", hoverColor: "hover:bg-plum-solid", iconBg: "bg-plum/30",
              },
              {
                label: "Closing Balance",
                value: `Rs. ${closingBal.toLocaleString()}`,
                icon: <Banknote size={20} />,
                color: "bg-info-solid", hoverColor: "hover:bg-info-solid", iconBg: "bg-info/30",
              },
              {
          ...
          <div className="bg-surface rounded-lg shadow-sm border border-line p-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-5 gap-3">
              <div>
                <h3 className="text-[18px] font-[600] text-fg">Ledger Entries</h3>
              ...
              <div className="flex items-center gap-3">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="pl-9 pr-4 py-2 border border-line rounded-lg text-[13px] focus:outline-none focus:ring-2 focus:ring-brand-fg focus:border-transparent w-60"
                    value={tableSearch}
                      ...
                      {paginated.length === 0 ? (
                        <tr>
                          <td colSpan={9} className="py-16 text-center">
                            ...
                            <p className="text-[14px] font-[500] text-fg">No ledger entries found</p>
                            <p className="text-[12px] text-fg-secondary">Try adjusting your filters</p>
                          </td>
                        </tr>
                      ) : paginated.map((row) => {
                        const typeStyle   = TYPE_STYLE[row.type]   || { text: "text-fg-secondary", bg: "bg-app" };
                        const statusStyle = STATUS_STYLE[row.status] || { text: "text-fg-secondary", bg: "bg-app", icon: null };
                        return (
                          <tr
                            key={row.id}
                            className={`border-b border-line hover:bg-subtle transition-colors ${row.status === "Overdue" ? "bg-error/10" : ""}`}
                          >
                            ...
                            <td className="py-3.5 px-4">
                              <button
                                ...
                                className="text-[13px] font-[600] text-brand-fg hover:underline whitespace-nowrap flex items-center gap-1"
                              >
                            ...
                            <td className="py-3.5 px-4 max-w-[200px]">
                              <p className="text-[12px] text-fg-secondary truncate" title={row.description}>
                                {row.description}
                              </p>
                            </td>
                            ...
                            <td className="py-3.5 px-4">
                              {row.debit > 0 ? (
                                <span className="text-[13px] font-[700] text-error">
                            ...
                            <td className="py-3.5 px-4">
                              <span className={`text-[13px] font-[700] ${row.balance === 0 ? "text-success" : row.status === "Overdue" ? "text-error" : "text-fg"}`}>
...
function ManualAdjustmentModal({ suppliers, preSelectedId, onClose, onSuccess }) {
  ...
  return (
    <div className="fixed inset-0 bg-backdrop z-[9999] flex items-center justify-center p-4 backdrop-blur-sm">
      <div className="bg-elevated rounded-xl shadow-2xl w-full max-w-lg overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          {error && (
            <div className="p-3 bg-error/10 border border-error/30 text-error text-[13px] rounded-lg flex items-center gap-2">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              ...
              <select
                ...
                className="w-full px-3 py-2 border border-line rounded-lg text-[13px] focus:ring-2 focus:ring-brand-fg outline-none"
              >
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-3 py-2 border border-line rounded-lg text-[13px] focus:ring-2 focus:ring-brand-fg outline-none resize-none"
            ></textarea>
          </div>
          ...
          <div className="pt-4 flex items-center justify-end gap-3 border-t border-line">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 text-fg-secondary text-[13px] font-[500] hover:bg-subtle rounded-lg transition-colors"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={loading}
              className="flex items-center gap-2 px-6 py-2 bg-brand text-on-brand text-[13px] font-[500] rounded-lg hover:bg-brand-hover transition-all disabled:opacity-50"
            >

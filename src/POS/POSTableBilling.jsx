...
export default function POSTableBilling() {
    ...
    const handleAddTable = async (e) => {
        ...
        try {
            ...
            const response = await axios.post(`/api/pos/v1/table-billing/tables`, requestBody);
    ...
    const handleOrderItemOrder = async (item) => {
        try {
            ...
            await axios.post(`/api/pos/v1/table-billing/add-item`, requestBody);
            toast.success(`Item ${item.name} added to table successfully!`);
            fetchTableDetails(); // Refresh billing details
    ...
    const fetchTables = async () => {
        try {
            const response = await axios.get(`/api/pos/v1/table-billing/tables`);
    ...
    const fetchTableDetails = async () => {
        ...
        try {
            const response = await axios.get(`/api/pos/v1/table-billing/table-details/${selectedTable}`);
    ...
    const removeOrderItem = async (itemId) => {
        try {
            await posService.removeTableItem(itemId);
            fetchTableDetails();
            fetchTodayItems();
        } catch (error) {
    ...
    const processPayment = async () => {
        ...
        try {
            ...
            fetchTables(); // Refresh tables status
            setCurrentOrder([]);
            fetchTodayItems(); // Refresh stock in product grid
            setSelectedTable('');
            setAmountReceived('');
    ...
    const handleTransferTable = async () => {
        ...
        try {
            await posService.transferTable(selectedTable, targetTableId);
    ...
    const handleUpdateTableStatus = async (newStatus) => {
        ...
        try {
            await posService.updateTableStatus(selectedTable, newStatus);
                                                ...
                                                <div className="flex-1 w-full">
                                                    ...
                                                    <div className="flex gap-2">
                                                        ...
                                                        >
                                                            ...
                                                            {tables
                                                                .filter(t => t.status === 'available' && String(t.id) !== String(selectedTable))
                                                                .map(t => (

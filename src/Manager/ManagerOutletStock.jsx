      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-8 overflow-y-auto">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
            <div>
              <h2 className="text-[20px] font-[600] text-fg">Live Outlet Stock</h2>
              <p className="text-[12px] text-fg-secondary">Monitor current stock quantities for each outlet</p>
            </div>
            ...
            <div className="flex items-center gap-3 w-full sm:w-auto">
              <select
                ...
                className="w-full sm:w-[200px] border border-line-strong rounded-lg px-3 py-2 focus:outline-none focus:border-brand-fg text-[14px]"
              >
              ...
              <div className="relative flex-1 sm:flex-none">
                ...
                <input
                  ...
                  className="w-full sm:w-[250px] pl-9 pr-4 py-2 border border-line rounded-lg text-[13px] focus:outline-none focus:ring-1 focus:ring-brand-fg"
                />
              </div>
              <button
                onClick={() => selectedOutlet && fetchOutletStock(selectedOutlet)}
                disabled={!selectedOutlet || loading}
                className={`p-2 rounded-lg border border-line text-fg-secondary hover:bg-subtle transition-colors ${(!selectedOutlet || loading) ? 'opacity-50 cursor-not-allowed' : ''}`}
                title="Refresh Stock"
              >
                  ...
                  {!selectedOutlet ? (
                    <tr>
                      <td colSpan="4" className="py-12 text-center">
                        <Store className="mx-auto text-fg-muted mb-2" size={32} />
                        <p className="text-fg-secondary text-[14px]">Please select an outlet from the dropdown</p>
                      </td>
                    </tr>
                  ) : loading ? (
                    <tr>
                    ...
                    <tr>
                      <td colSpan="4" className="py-8 text-center text-error text-[14px]">
                        {error}
                      </td>
                    </tr>
                      ...
                      >
                        <td className="py-4 px-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <span className="block text-[14px] font-[500] text-fg">{item.productName}</span>
                              <span className="block text-[12px] text-fg-muted">{item.productCode}</span>
                            </div>
                          </div>
                        </td>
                        <td className="py-4 px-4">
                          <span className="text-[13px] text-fg-secondary">{item.categoryName}</span>
                        </td>
                        <td className="py-4 px-4 text-center">
                          <span className="text-[14px] font-[500] text-fg-secondary">{item.receivedQty}</span>
                        </td>
                        <td className="py-4 px-4 text-center">
                          <span className="text-[14px] font-[600] text-brand-fg">{item.currentQty}</span>
                        </td>
                      </tr>
                    ))
                  )}
